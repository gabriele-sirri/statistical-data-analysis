...
- “Label”, since it is used to distinguish the signal sample from the background sample. This variable would not be available in a real dataset from a real experiment (!).
...
the macro called by the “(5a) Classifier Cut Efficiencies” button of TMVAGUI compute a significance using an approximated formula 

$ Z = \sqrt{S /(S+B )} $ 

 ($S$ – number of signal, $B$ – number of background).
